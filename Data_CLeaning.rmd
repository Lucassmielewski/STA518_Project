...
(game_df <- game_df %>% 
  mutate(`First` = case_when(row_number() %in% new_inning ~ 0,
                              lag(`Batter Bases`==1) & (is.na(`Runner Advancements`)) ~ 1,
                              lag(`Batter Bases`==4) & (is.na(`Runner Advancements`)) ~ 0),
...
```{r Moving Runners}
game_df %>% 
  mutate(`Second` = case_when(lag(str_detect(`Runner Advancements`, "-2")) ~ 1, 
                              TRUE ~ as.numeric(Second)),
         `Third` = case_when(lag(str_detect(`Runner Advancements`, "-3")) ~ 1, 
                              TRUE ~ as.numeric(Third)),
         `Runs` = case_when(lag(str_detect(`Runner Advancements`, "-H")) ~ 
                              lag(str_count(`Runner Advancements`,"-H"))))
```
