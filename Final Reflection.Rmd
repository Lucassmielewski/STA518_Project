...
get_results <- function(game_codes){
  ...
  game_df <- game_df %>% 
    ...
    separate(Outcome, into = c('Outcome', 'Runner Advancements'),"[.]", extra = "merge") %>%  
  ...
  batter_conditions <- map(names(batter_codes), 
                           ~quo(str_starts(Outcome, 
                                           regex(paste(!!.x,"+([0-9]|/|$)|E|(W+)|(SBH;)", 
                                                       sep="")))~batter_codes[!!.x]))
...
Finally I get to the most intriguing part of my project, building an interactive Shiny app. Even though I was able to produce a simple app producing all of the same concepts that I had set out to do at the start of my project, it gives me error when opening the application from the shinyapps.io server, so this is something I will have to investigate further. Nevertheless my Shiny app will run and be produced correctly when launched from my Rstudio session the code for this can be seen below. 
